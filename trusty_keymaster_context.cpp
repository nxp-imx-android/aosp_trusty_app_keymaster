...
keymaster_error_t TrustyKeymasterContext::LoadHbk(
        uint8_t *key,
        uint32_t *key_size) {
    const char* hbk_key_id = "com.android.trusty.keymaster.hbk";
    assert(key != NULL);

    long rc = hwkey_open();
    if (rc < 0) {
        LOG_E("hwkey_open failed\n", 0);
        return KM_ERROR_UNKNOWN_ERROR;
    }

    hwkey_session_t session = (hwkey_session_t)rc;
    rc = hwkey_get_keyslot_data(session, hbk_key_id, key, key_size);
    if (rc < 0) {
        LOG_E("Get hardware bound key failed! Err: %d\n", rc ,0);
        return KM_ERROR_UNKNOWN_ERROR;
    }

    assert(*key_size == kHardwareBoundKeySize);

    hwkey_close(session);
    return KM_ERROR_OK;
}

keymaster_error_t verifyID(
        const uint8_t *id,
        uint32_t length) {
    ...
    keymaster_error_t err = ss_manager->ReadAttestationID(&attestation_id_p);
    if (err != KM_ERROR_OK) {
        return err;
    }

    // Sanity check the id hmac length
    UniquePtr<AttestationID> attestation_id(attestation_id_p);
    uint32_t id_hmac_size = attestation_id->hmac_id.size;
    if ((id_hmac_size == 0) ||
        (id_hmac_size % SHA256_DIGEST_LENGTH != 0)) {
        LOG_E("Error! The ID hmac length is wrong!", 0);
    ...
    err = TrustyKeymasterContext::LoadHbk(hbk, &hbk_size);
    if (err != KM_ERROR_OK) {
        LOG_E("Error: failed to load hardward bound key, err: %d\n", err);
        return err;
    }

    // Verify the whole ID hmac
    KeymasterBlob hmac_out;
    keymaster_key_blob_t hamc_key = {
            reinterpret_cast<const uint8_t*>(hbk),
            kHardwareBoundKeySize};
    keymaster_blob_t data = {
            reinterpret_cast<const uint8_t*>(attestation_id->hmac_id.bytes),
            id_hmac_size};
    keymaster_blob_t data_chunks[] = {data};
    err = OpenSSLKeymasterEnforcement::hmacSha256(hamc_key, data_chunks, 1, &hmac_out);
    if (err != KM_ERROR_OK) {
    ...
    if (memcmp(hmac_out.data, attestation_id->hmac_all.bytes, SHA256_DIGEST_LENGTH)) {
        LOG_E("Error: Device IDs are corrupted!\n", 0);

        return KM_ERROR_UNKNOWN_ERROR;
    }

    // Verify if the given ID matches any provisioned IDs
    ...
    uint8_t *id_hmac_data = attestation_id->hmac_id.bytes;
    for (int i = 0; i < (int)(id_hmac_size/SHA256_DIGEST_LENGTH); i++) {
        if (!memcmp(hmac_out.data, id_hmac_data + (i * SHA256_DIGEST_LENGTH), SHA256_DIGEST_LENGTH))
            return KM_ERROR_OK;
    }

    LOG_E("Error: Device ID not found!\n", 0);
    // Return error if no provisioned ID match
    return KM_ERROR_UNKNOWN_ERROR;
}

template <keymaster_tag_t Tag>
keymaster_error_t AttestDeviceID (
        const AuthorizationSet& attestation_params,
        AuthorizationSet* attestation,
        TypedTag<KM_BYTES, Tag> tag,
        keymaster_blob_t* val) {
    if (attestation_params.GetTagValue(tag, val)) {
        if (verifyID(val->data, val->data_length) == KM_ERROR_OK) {
            attestation->push_back(tag, val->data, val->data_length);
            return KM_ERROR_OK;
        } else {
            LOG_E("Attest device ID failed!", 0);
            return KM_ERROR_CANNOT_ATTEST_IDS;
        }
    } else {
        LOG_E("Get device ID failed!", 0);
        return KM_ERROR_CANNOT_ATTEST_IDS;
    }
}
...
keymaster_error_t TrustyKeymasterContext::VerifyAndCopyDeviceIds (
    ...
    bool deviceIdIncluded = false;
    keymaster_error_t ret;

    // Check if device id is included or not
    for (const auto& tag : kDeviceAttestationTags) {
        if (attestation_params.find(tag) != -1) {
            deviceIdIncluded = true;
            break;
        }
    }
    if (!deviceIdIncluded) {
        LOG_I("Device ID attestation is not included!", 0);
        return KM_ERROR_OK;
    }
    keymaster_blob_t identifiers;

    // Brand name, must be present
    ret = AttestDeviceID(attestation_params, attestation,
                         TAG_ATTESTATION_ID_BRAND, &identifiers);
    if (ret != KM_ERROR_OK) {
        LOG_E("Attest TAG_ATTESTATION_ID_BRAND failed!", 0);
        return ret;
    }

    // Device name, must be present
    ret = AttestDeviceID(attestation_params, attestation,
                         TAG_ATTESTATION_ID_DEVICE, &identifiers);
    if (ret != KM_ERROR_OK) {
        LOG_E("Attest TAG_ATTESTATION_ID_DEVICE failed!", 0);
        return ret;
    }

    // Product name, must be present
    ret = AttestDeviceID(attestation_params, attestation,
                         TAG_ATTESTATION_ID_PRODUCT, &identifiers);
    if (ret != KM_ERROR_OK) {
        LOG_E("Attest TAG_ATTESTATION_ID_PRODUCT failed!", 0);
        return ret;
    }

    // Manufacturer name, must be present
    ret = AttestDeviceID(attestation_params, attestation,
                         TAG_ATTESTATION_ID_MANUFACTURER, &identifiers);
    if (ret != KM_ERROR_OK) {
        LOG_E("Attest TAG_ATTESTATION_ID_MANUFACTURER failed!", 0);
        return ret;
    }

    // Model name, must be present
    ret = AttestDeviceID(attestation_params, attestation,
                         TAG_ATTESTATION_ID_MODEL, &identifiers);
    if (ret != KM_ERROR_OK) {
        LOG_E("Attest TAG_ATTESTATION_ID_MODEL failed!", 0);
        return ret;
    }

    // Serial number, optional
    if (attestation_params.find(TAG_ATTESTATION_ID_SERIAL) != -1) {
        ret = AttestDeviceID(attestation_params, attestation,
                             TAG_ATTESTATION_ID_SERIAL, &identifiers);
        if (ret != KM_ERROR_OK) {
            LOG_E("Attest TAG_ATTESTATION_ID_SERIAL failed!", 0);
            return ret;
        }
    }

    // IMEIs, optional
    if (attestation_params.find(TAG_ATTESTATION_ID_IMEI) != -1) {
        ret = AttestDeviceID(attestation_params, attestation,
                             TAG_ATTESTATION_ID_IMEI, &identifiers);
        if (ret != KM_ERROR_OK) {
            LOG_E("Attest TAG_ATTESTATION_ID_IMEI failed!", 0);
            return ret;
        }
    }

    // MEIDs, optional
    if (attestation_params.find(TAG_ATTESTATION_ID_MEID) != -1) {
        ret = AttestDeviceID(attestation_params, attestation,
                             TAG_ATTESTATION_ID_MEID, &identifiers);
        if (ret != KM_ERROR_OK) {
            LOG_E("Attest TAG_ATTESTATION_ID_MEID failed!", 0);
            return ret;
        }
    }
